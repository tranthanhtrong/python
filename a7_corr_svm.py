...
def correction_svm(filenameTrain, resultColName, fileListTest, nTimes, percentTest, coef_percent):
    print(textcolor_display(str("Train by " + str(filenameTrain) + ", run test " + str(nTimes) + " times, use " + str(
        percentTest) + " percent from train to get coef and coef > " + str(coef_percent)), color.HEADER))
